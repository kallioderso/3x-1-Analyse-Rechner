...
{
    public static void Calculate(Int64 Zahl, int Zyklen)//Method to Calculate the Current Number
    {
        Int64 Eingabezahl = Zahl;//var for the Current Number
        ...
        for (int i = 0; i < Zyklen; i++)//loop to Calculate 3x+1 with the current number 
        {
            ...
            {
                Int64 NeueZahl = (Zahl / 2);//Divided the current number by 2
            ...
            {
                Int64 NeueZahl = ((Zahl * 3) + 1); //Calculate the Current Number with 3x+1
                Zahl = NeueZahl; // set created number to the Number
                ZyklenCount++; //Add 1 to the count for the Zyklus
            }
        }
        
        var Ergebnis = (Eingabezahl + " -> " + Zahl + " = " + ZyklenCount + " Zyklen");//creates string for File
